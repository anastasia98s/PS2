...
def listen(silence_duration, sample_rate, max_time=config.MAX_RECORDING_TIME, status_class_thread=None, save_rec=False):
    ...
    if save_rec:
        try:
            ...
            with wave.open(config.AUDIO_TMP_PATH, 'wb') as wf:
                wf.setnchannels(1)
                wf.setsampwidth(2)
