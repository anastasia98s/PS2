...
class Engine:
    ...
    def authentifizieren(self, signal, status_class_thread=None):
        if (status_class_thread and status_class_thread.thread_event.is_set()) or not self.benutzer_id:
            ...
            if pred_id:
                ...
                if error_request:
                    return pred_name, 1
                antwort_text = self.speech_to_text.dialog(0.5, "Sind Sie " + pred_name, status_class_thread=status_class_thread)
                if antwort_text and any(word in antwort_text.lower().split() for word in ["ja", "genau"]):
                    return pred_id, None
            
            if isinstance(name_label, list) and len(name_label) > 0 and len(name_label[0]) > 1:
                antwort_text = self.speech_to_text.dialog(0.5, "Haben Sie bereits ein Konto?", status_class_thread=status_class_thread)
                if antwort_text and any(word in antwort_text.lower().split() for word in ["ja", "genau"]):
                    for label in name_label[0]:
                        ...
                        if error_request:
                            return geg_name, 1
                        antwort_text = self.speech_to_text.dialog(0.5, "Sind Sie " + geg_name, status_class_thread=status_class_thread)
                        if antwort_text and any(word in antwort_text.lower().split() for word in ["ja", "genau"]):
                            return geg_id, None

            antwort_text = self.speech_to_text.dialog(0.5, "Wollen Sie ein Konto erstellen?", status_class_thread=status_class_thread)
            if antwort_text and any(word in antwort_text.lower().split() for word in ["ja", "okay"]):
                antwort_text = self.speech_to_text.dialog(0.5, "Wie heißt du?", status_class_thread=status_class_thread)
                if antwort_text:
    ...
    def start(self):
        ...
        while True:
            ...
            main_thread = threading.Thread(target=engine_processor.start, args=(wake_word_signal,))
            main_thread.start()
