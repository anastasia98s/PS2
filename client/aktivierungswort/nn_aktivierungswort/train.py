...
def train():
    try:
        ...
        merkmale, labels = aktivierungswort.nn_aktivierungswort.utils.get_data(config.AKTIVIERUNGSWORT_DATENBANK_PATH)
        ...
        if len(merkmale > 1):
            ...
            train_dataset = aktivierungswort.nn_aktivierungswort.utils.MerkmaleDataset(train_merkmale, train_labels)
            train_data_loader = DataLoader( train_dataset,
                                            batch_size=config.AKTIVIERUNGSWORT_TRAIN_BATCH_SIZE,
                                            shuffle=True,
                                            pin_memory=True)
            
            val_dataset = aktivierungswort.nn_aktivierungswort.utils.MerkmaleDataset(val_merkmale, val_labels)
            ...
            for epoch in range(config.AKTIVIERUNGSWORT_EPOCHS):
                train_loss = aktivierungswort.nn_aktivierungswort.utils.train_fn(train_data_loader, model, optimizer, device)
                val_loss, preds_aktivierung_array, loesung_aktivierung_array = aktivierungswort.nn_aktivierungswort.utils.val_fn(val_data_loader, model, device)
                ...
                if val_loss < best_loss or epoch % 100 == 0 or epoch == config.AKTIVIERUNGSWORT_EPOCHS-1:
                    ...
                    aktivierungswort.nn_aktivierungswort.utils.show_conf_matrix(preds_aktivierung_array, loesung_aktivierung_array, confusion_matrix_class, "Aktivierung-KI", binary=True)
