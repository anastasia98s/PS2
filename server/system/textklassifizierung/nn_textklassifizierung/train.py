...
def train():
    try:
        ...
        for epoch in range(config.TEXTKLASSIFIZIERUNG_EPOCHS):
            ...
            val_loss, raw_preds_anmerkung_array, raw_loesung_anmerkung_array, preds_absicht_array, loesung_absicht_array, preds_szenario_array, loesung_szenario_array = nn_textklassifizierung.utils.val_fn(val_data_loader, model, device)
            ...
            if val_loss < best_loss and config.TEXTKLASSIFIZIERUNG_SAVE_MODEL:
                ...
                nn_textklassifizierung.utils.show_conf_matrix(preds_anmerkung_array, loesung_anmerkung_array, confusion_matrix_anmerkung_class, "Textklassifizierung-KI (Anmerkung)")
                nn_textklassifizierung.utils.show_conf_matrix(preds_absicht_array, loesung_absicht_array, confusion_matrix_absicht_class, "Textklassifizierung-KI (Absicht)")
                nn_textklassifizierung.utils.show_conf_matrix(preds_szenario_array, loesung_szenario_array, confusion_matrix_szenario_class, "Textklassifizierung-KI (Szenario)")
        ...
        nn_textklassifizierung.utils.show_conf_matrix(flat_preds_anmerkung_array, flat_loesung_anmerkung_array, confusion_matrix_anmerkung_class, f"{best_epochs} Epochs | Total: {len(flat_preds_anmerkung_array)} | Textklassifizierung-KI (Anmerkung)", plot=True)
        nn_textklassifizierung.utils.show_conf_matrix(best_preds_absicht_array, best_loesung_absicht_array, confusion_matrix_absicht_class, f"{best_epochs} Epochs | Total: {len(best_preds_absicht_array)} | Textklassifizierung-KI (Absicht)", plot=True)
        nn_textklassifizierung.utils.show_conf_matrix(best_preds_szenario_array, best_loesung_szenario_array, confusion_matrix_szenario_class, f"{best_epochs} Epochs | Total: {len(best_preds_szenario_array)} | Textklassifizierung-KI (Szenario)", plot=True)
        ...
        nn_textklassifizierung.utils.show_conf_matrix(absicht_szenario_vorhersage, absicht_szenario_loesung, absicht_szenario_label, f"{best_epochs} Epochs | Total: {len(absicht_szenario_vorhersage)} | Textklassifizierung-KI (Absicht + Szenario)", plot=True)
