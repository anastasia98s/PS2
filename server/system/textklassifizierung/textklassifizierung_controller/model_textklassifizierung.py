...
class ModelTextklassifizierung:
    ...
    def create_table(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS sp_verlauf (
                ...
                note INTEGER,
                FOREIGN KEY (szenario_id) REFERENCES sp_szenario(szenario_id) ON DELETE CASCADE ON UPDATE CASCADE,
                FOREIGN KEY (absicht_id) REFERENCES sp_absicht(absicht_id) ON DELETE CASCADE ON UPDATE CASCADE
            );
        ''')
    ...
    def add_verlauf(self, szenario, absicht, eingabe, ausgabe, note):
        conn = self.connect_db()
        try:
            cursor = conn.cursor()
            cursor.execute("PRAGMA foreign_keys = ON")
            cursor.execute('INSERT INTO sp_verlauf (szenario_id, absicht_id, eingabe, ausgabe, note) VALUES (?, ?, ?, ?, ?)', (szenario, absicht, eingabe, ausgabe, note))
            conn.commit()
