...
def train():
    try:
        ...
        merkmale, benutzerids, encoder_benutzerids = nn_authentifizierung.utils.get_data(config.USER_DATENBANK_PATH)
        if len(merkmale > 1):
            meta_data = {
            ...
            train_dataset = nn_authentifizierung.utils.MerkmaleDataset(train_merkmale, train_benutzerids)
            train_data_loader = DataLoader( train_dataset,
                                            batch_size=config.AUTHENTIFIZIERUNG_TRAIN_BATCH_SIZE,
                                            shuffle=True,
                                            pin_memory=True)
            
            val_dataset = nn_authentifizierung.utils.MerkmaleDataset(val_merkmale, val_benutzerids)
            ...
            user_database = ModelUser()
            for benutzerid in encoder_benutzerids.classes_:
                confusion_matrix_class.append(user_database.show_benutzer_name(benutzerid))
            ...
            for epoch in range(config.AUTHENTIFIZIERUNG_EPOCHS):
                ...
                val_loss, preds_authentifizierung_array, loesung_authentifizierung_array = nn_authentifizierung.utils.val_fn(val_data_loader, model, device)
                ...
                if val_loss < best_loss or epoch % 100 == 0 or epoch == config.AUTHENTIFIZIERUNG_EPOCHS-1:
                    ...
                    if val_loss < best_loss and config.AUTHENTIFIZIERUNG_SAVE_MODEL:
                        ...
                        nn_authentifizierung.utils.show_conf_matrix(preds_authentifizierung_array, loesung_authentifizierung_array, confusion_matrix_class, "Authentifizierung-KI")
