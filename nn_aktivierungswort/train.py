...
def train():
    merkmale, labels = nn_aktivierungswort.utils.get_data(config.AKTIVIERUNGSWORT_DATENBANK_PATH)
    ...
    if len(merkmale > 1):
        ...
        if config.AKTIVIERUNGSWORT_RETRAIN_MODEL:
            try:
                model.load_state_dict(torch.load(config.AKTIVIERUNGSWORT_TRAINED_PATH))
                print("\n!!!Retraining!!!")
        ...
        for epoch in range(config.AKTIVIERUNGSWORT_EPOCHS):
            train_loss = nn_aktivierungswort.utils.train_fn(train_data_loader, model, optimizer, device)
            val_loss = nn_aktivierungswort.utils.val_fn(val_data_loader, model, device)
        ...
        print("Zu wenige Daten, um Aktivierungswort-KI zu trainieren")